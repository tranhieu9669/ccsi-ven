      ...
      <h2>
        <?php 
          if(isset($result_rp['total']) AND $result_rp['total']){
            echo number_format(($result_rp['online']/$result_rp['total']), 2) . '%';
          }else{
            echo '##.##%';
          }
        ?>
      ...
      <h2>
        <?php 
          if(isset($result_rp['online']) AND $result_rp['online']){
            echo number_format(($result_rp['connected']/$result_rp['online']), 2) . '%';
          }else{
            echo '##.##%';
          }
        ?>
...
<script type="text/javascript">
  function load_content(){
    $.ajax({
      url     : '<?php echo base_url();?>monitor/load',
      dataType: 'html',
      type    : 'GET',
      data    : {},
      success : function(response, status, jqXHR){
          $('#content-agent').html(response);
      },
  ...
  $( document ).ready(function() {
    setInterval(function(){load_content()},5000);
  });
</script>