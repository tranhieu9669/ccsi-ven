...
<form id="call_log" method="POST" action="<?php echo $_SERVER['REQUEST_URI'];?>" autocomplete="off">
				...
				$('#dial_call').click(function(){
					...
					$.ajax({
				      	...
				      	success : function( data ) {
				      		console.log(data);
				      		//$('#dial_call').css('display', 'none');
				      		
				      		if( data == 'SUCCESS' ){
				      			$('#dial_call').css('display', 'none');
				      		}else{
				      			alert('Không gọi được tới khách hàng');
				      		}
				      	},
				      	error   : function(msg){
				        	alert('Lỗi gọi hệ thống');
				      	}
				    });
				});
			});
		...
		<div class="col-sm-12">
			<div class="panel panel-primary">
				<div class="panel-heading">
					...
					<?php
					if(isset($istype) AND !empty($istype)){
						if( in_array($istype, array('call', 'callback', 'appointment', 'recall')) ){?>
					<button type="submit" name="submit_save_data" class="btn btn btn-success dial" value="123">
						<span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span></span> Save
					</button>

					<button type="submit" name="submit_cancel_data" class="btn btn-warning dial" href="<?php echo base_url().'12345-dial'; ?>" target="_blank">
						<span class="glyphicon glyphicon-phone-alt" aria-hidden="true"></span> Cancel
					</button>

					<button type="button" class="btn btn-danger dial" id="dial_call">
					    ...
					    <script id="toolbar_template_introduc" type="text/x-kendo-template">
					    	<div>
					            ...
					            <span class="pull-right">
					                <ul class="fieldlist" style="float:left;">
					                    <li>
				                        	<a _modal="dialog_infor_detail" _title="Thêm thông tin khách hàng giới thiệu" href="<?php echo base_url() . $customerdt['id'];?>-addintroduc" class="sys_modal"><i class="fa fa-plus-square-o"></i>Thêm thông tin</a>
					                    </li>
					                </ul>
					            </span>
					        </div>
			...
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<div class="col-sm-6 box-item">
						<div class="col-sm-12">
							<div class="form-group">
								<input type="hidden" id="hd_id_call_status" name="hd_id_call_status" value="">
					...
					<div class="col-sm-6 box-item">
						<div class="col-sm-12">
							<div class="form-group">
						  	...
						  	<div class="form-group">
							    <label class="control-label col-sm-4" for="pwd">Thành phố </label>
							    <div class="col-sm-8 box-row">
							    ...
							    <script type="text/javascript">
						      		$(function(){
								        ...
								        $("#appointment").change(function(){
											...
											if(dateapp != ''){
												var _dateapp = dateapp.split("/");
												dateapp = _dateapp[2]+'-'+_dateapp[1]+'-'+_dateapp[0];

												$.ajax({
											      	url     : 'ajax/frametimeappointment',
											      	type    : "GET",
					...
					<div class="col-sm-6 box-item">
						<div class="col-sm-12">
							<div class="form-group">
						  	...
						  	<div class="form-group">
						  		<label class="control-label col-sm-4 padding-left-15" for="pwd">Chi nhánh </label>
							    <div class="col-sm-8 box-row">
	...
	<!-- End Schedule -->
	<!-- History call -->
	<?php if( in_array($this->_role, array('supadmin', 'admin', 'center', 'department', 'group')) ){ ?>
	<div class="container-fluid box-infor" id="history_info">
		<div class="col-sm-12">
			<div class="panel panel-primary">
				<div class="panel-heading">
				...
				<div class="panel-body">
					<div class="col-sm-12">
						<script type="text/javascript">
					        var url_history = "<?php echo base_url() . $customerdt['id'];?>-history";
					        $(function() {
					            ...
					            var grid_history_config = {
					                ...
					                'columns': [
					                    { field: "created_at", title: 'Thời gian', width: column_history_properties._wth_time, sortable: false,},
					                    { field: "agent_ext", title: 'Máy lẻ', width: column_history_properties._wth_ext, sortable: false,},
					                    { field: "source", title: 'Nguồn', width: 100, width: column_history_properties._wth_source, sortable: false, },
					                    { field: "sname", title: 'Trạng thái', width: column_history_properties._wth_status, sortable: false,},
					                    { field: "content_call", title: 'Nghi chú', sortable: false,},
					                ]
					            };
...
<script type="text/javascript">
	$(function(){
		$( "form" ).submit(function( event ) {
			...
			var type_status = $('#id_call_status option:selected').attr('type');
		...
		$(".date-picker").kendoDatePicker({
	  	...
	  	$("#id_call_status").change(function(){
	  		...
	  		var hd_status_type = $('#id_call_status option:selected').attr('type');
	  		$('#hd_id_call_status').val(hd_status_type);

	  		if(hd_status_type == 'appointment'){
