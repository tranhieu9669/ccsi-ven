...
class Upload extends MY_Controller {
    ...
    function checkdup($mobile=false, $id_fileup=false){
    	...
    	if( isset($detail) AND !empty($detail) ){
			...
			$this->db->insert('tbl_customer_duplicate', $datadup);
    ...
    */
    ...
    function extractexcel($id=false){
    	...
    	$uploaddt = array();
        $this->db->select('id,filename,id_source,source_name');
        $this->db->where('id', $id);
            ...
            if( file_exists($filepath) ){
                ...
                for ($row = 2; $row <= $max_row; $row++) {
                    ...
                    if( strlen($mobile) >= 10 ){
                        ...
                        if( ! $this->checkdup($mobile, $id) AND ! in_array($mobile, $data_check) ){
                ...
                if( isset($data_insert) AND !empty($data_insert) ){
                    if( $this->db->insert_batch('tbl_customer', $data_insert) !== FALSE ){
                        $this->db->where('id', $id);
