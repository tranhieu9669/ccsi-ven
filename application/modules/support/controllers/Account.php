...
class Account extends MY_Controller {
    ...
    function getext($id_center = false, $id_department = false){
        ...
        $detail = array();
        $this->db->select('ext');
        if(strlen($first_ext) > 1){
            $this->db->where('LENGTH(ext) = 5');
        }else{
            $this->db->where('LENGTH(ext) = 4');
        }
        $this->db->like('ext', $first_ext, 'after'); #before
        $this->db->order_by('ext', 'asc');
        $detail = $this->db->get('tbl_accounts')->result_array();
    ...
    function agentcrm($fields){
        $url_api = 'http://192.168.1.48:8088/api/Agent';
        $ch = curl_init();
    ...
    function detail($id=0){
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            if( $action == 'dataaction' ){
                ...
                $sipserver  = $this->input->post('sipserver');
                ...
                if( !in_array($sipserver, array('192.168.1.17','192.168.1.14')) ){
                    $validation[] = array(
                        'field' => 'ext_extend',
                        'label' => 'Số Ext',
                        'rules' => 'required|max_length[4]|is_unique[tbl_accounts.ext_extend]',
                    );
                }
                ...
                if ( $this->form_validation->run() !== FALSE ){
                    $hd_center      = $this->input->post('hd_center');
                    $id_center      = $this->input->post('id_center');
                    $hd_department  = $this->input->post('hd_department');
                    $id_department  = $this->input->post('id_department');
                    $hd_group       = $this->input->post('hd_group');
                    $id_group       = $this->input->post('id_group');
                    $num_data       = $this->input->post('num_data');
                    $id_city        = $this->input->post('id_city');
                    $full_name      = trim($this->input->post('full_name'));
                    $first_name     = '';
                    $last_name      = '';
                    $ext_extend     = $this->input->post('ext_extend');
                    ...
                    $sipname    = 'deaura';
                    if( ! in_array($sipserver, array('192.168.1.17','192.168.1.14','192.168.1.93')) ){
                        $sipname = 'caresoft';
                    }
                    ...
                    if( ! in_array($sipserver, array('192.168.1.17','192.168.1.14')) ){
                        if($ext_extend){
                            $accounts['ext_extend'] = $ext_extend;
                        }
                    }else{
                        $accounts['ext_extend'] = '';
                    }
                    ...
                    if( ! $flag ){
                        ...
                        # Update Log Call
                        if($id_group AND $id_group != $hd_group AND $num_data){
                            #hd_center
                            #id_center
                            #hd_department
                            #id_department
                            ...
                            # Xoa toan bo du lieu tren cac phong ban cu
                            # Cap nhat call log
                            # lay lai thong tin khach hang tu kho tong
                        }
                    }
                    ...
                    if ($this->db->trans_status() === FALSE){
                        ...
                        $this->db->where('id', $id);
                        $crmdt = $this->db->get('tbl_accounts')->row_array();
                        if(isset($crmdt) AND !empty($crmdt)){
                            $fields = array(
                                'extension' => $crmdt['ext'],
                                'fullname' => $crmdt['full_name'],
                            );
                            $this->agentcrm($fields);
                        }
                    }
                }
            }else{
