...
<form id="formSubmit" class="form-horizontal" role="form" method="POST" action="<?php echo $_SERVER['REQUEST_URI'];?>" autocomplete="off">
    ...
    <?php
      $sipserver = '192.168.1.17';
      if(isset($detail['sipserver']) AND !empty($detail['sipserver'])){
        $sipserver = $detail['sipserver'];
      }
    ?>
    <div class="form-group">
      <label class="control-label col-sm-3 dialog-control" for="pwd">SIP Server </label>
      <div class="col-sm-9 dialog-control">
        <select id="sipserver" name="sipserver" class="form-control-select">
          <option value="192.168.1.17" <?php if($sipserver == "192.168.1.17"){ echo "selected='selected'"; } ?> > Deaura 17</option>
          <option value="192.168.1.14" <?php if($sipserver == "192.168.1.14"){ echo "selected='selected'"; } ?> > Deaura 14</option>
		  <option value="192.168.1.93" <?php if($sipserver == "192.168.1.93"){ echo "selected='selected'"; } ?> > Deaura 93</option>
          <option value="vip.caresoft.vn:5063" <?php if($sipserver == "vip.caresoft.vn:5063"){ echo "selected='selected'"; } ?> > Caresoft</option>
        </select>
      </div>
    </div>

    <div class="form-group">
      <label class="control-label col-sm-3 dialog-control" for="pwd">Ext khác </label>
      <div class="col-sm-9 dialog-control">
        <input type="text" name="ext_extend" class="form-control" placeholder="Ext Extend" value="<?php echo set_value_input('ext_extend', $detail);?>">
        <span class="error"><?php echo form_error('ext_extend'); ?></span>
      </div>
    </div>
