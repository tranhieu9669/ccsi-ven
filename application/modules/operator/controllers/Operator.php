...
class Operator extends MY_Controller {
    ...
    function index(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $data_out = array();
            $this->dbop->select('src, count(1) as total, SUM(CASE WHEN disposition="ANSWERED" AND (dcontext LIKE "qm-queuedial%" OR dcontext = "subscribers_oper" OR dcontext = "subscribers_cs") THEN 1 ELSE 0 END) as ans, SUM(CASE WHEN disposition="ANSWERED" AND (dcontext LIKE "qm-queuedial%" OR dcontext = "subscribers_oper" OR dcontext = "subscribers_cs") THEN billsec ELSE 0 END) as bill');
            ...
            $this->dbop->from('bit_cdr');
            $this->dbop->where('dst', 'main');
            $this->dbop->like('dstchannel', 'SIP/', 'after');
    ...
    function exoperator(){
        ...
        if(isset($dataResult) AND !empty($dataResult)){
            ...
            foreach ($dataResult as $key => $value) {
                ...
                $objWorksheet->setCellValue('F'.$row, $total_ans);
                $objWorksheet->setCellValue('G'.$row, $bill_out);
                $objWorksheet->setCellValue('H'.$row, $total_in);
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if($calltype=='out'){
                ...
                if(isset($inputsearch) AND !empty($inputsearch)){
                    $this->dbop->where('(src like "%'.$inputsearch.'%" OR lastdata like "%'.$inputsearch.'%" OR dst like "%'.$inputsearch.'%")');
                }
            }else{
				$haha = implode(" ",$this->agop);
				$hehe = substr($haha, 0, 2);
				$this->dbop->where('dst', 'main');
                $this->dbop->like('dstchannel', 'SIP/'.$hehe, 'after');
    ...
    function exportdetail($calltype='out'){
        $dataResult = array();
        # request Param
        $request    = $_REQUEST;
        $inputsearch = isset($request['inputsearch']) ? $request['inputsearch'] : false;
        ...
        $this->dbop->where('calldate >', $startdate . ' 07:00:00');
        $this->dbop->where('calldate <', $enddate . ' 23:59:00');
        $dataResult = $this->dbop->get()->result_array();
