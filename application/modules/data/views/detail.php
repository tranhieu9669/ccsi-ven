...
<form id="formSubmit" class="form-horizontal form-top" action="<?php echo $_SERVER['REQUEST_URI'];?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label class="control-label col-sm-2 dialog-control" for="text1">Type</label>
        <div class="col-sm-3 dialog-control">
            <select id="id_source_type" name="id_source_type" class="form-control-select">
        ...
        <label class="control-label col-sm-2 dialog-control" for="text1">Source</label>
        <div class="col-sm-5 dialog-control">
    ...
    <div class="form-group">
        <label class="control-label col-sm-2 dialog-control" for="text1">Time Ex</label>
        <div class="col-sm-10 dialog-control">
            <input type="text" name="schedule" id="schedule" class="form-control date-picker" placeholder="Thời gian đọc" >
        </div>
    </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $(".date-picker").kendoDateTimePicker({
        ...
        $('#id_source_type').change(function(){
            var _type = $(this).val();
            $.ajax({
                url     : '<?php echo base_url();?>data/Ajax/getsourcebytype/' + _type,
                type    : "GET",
                data    : {'type':_type},
                success : function( data ) {
                    var obj_decode = $.parseJSON(data);
                    var source = obj_decode;
                  
                    var html_source = '<option value="" selected="selected"> Chọn nguồn</option>';
                    for(var key in source){
                        var detail  = source[key];
                        var id      = detail['id'];
                        var name    = detail['name'];
                    html_source += '<option value="' + id + '">' + name + '</option>';
                }

                $('#id_source').html(html_source);
                $("#id_source").select2("val", "");
            },
