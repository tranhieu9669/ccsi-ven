...
class Except extends MY_Controller {
    ...
    function AddCommandos($mobile=false){
    	...
    	if(isset($detail) AND !empty($detail)){
			...
			# insert center
			$db_com_center = $this->setdbconnect($com_id_center, 'center');
			if($db_com_center){
                $db_com_center->where('mobile', $mobile);
                $db_com_center->delete('tbl_customer');
                ######
				$dbInsertCenter = array(
				 	'id_link' => $id,
				 	'id_fileup' => $id_fileup,
				...
				$db_com_center->insert('tbl_customer', $dbInsertCenter);
			}
	    	# insert department
	    	$db_com_department = $this->setdbconnect($com_id_department, 'department');
	    	if($db_com_department){
                $db_com_department->where('mobile', $mobile);
                $db_com_department->delete('tbl_customer');
                #####
				$dbInsertDepartment = array(
				 	'id_link' => $id,
				 	'id_fileup' => $id_fileup,
					'first_name' => $first_name,
					'last_name' => $last_name,
				...
				$db_com_department->insert('tbl_customer', $dbInsertDepartment);
			}
	    	# insert group
