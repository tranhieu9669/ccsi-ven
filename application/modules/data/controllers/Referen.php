...
class Referen extends MY_Controller {
    ...
    function index(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            #### select fullname,mobile,employeeCode,employeeInput from tbl_customer where employeeCode is not null
            $this->db->start_cache();
            $this->db->select('fullname,mobile,employeeCode,employeeInput');
            $this->db->from('tbl_customer');
            $this->db->where('employeeCode is not null');
            $this->db->order_by('employeeInput', 'DESC');
            ...
            if( file_exists($filepath) ){
                ...
                for ($row = 2; $row <= $max_row; $row++) {
                    ...
                    if( $this->checkMobile($mobile) ){
                        ...
                        $employeeCode = trim($objPHPExcel->getActiveSheet()->getCell("F" . $row)->getCalculatedValue());
                        ...
                        if($checkCus){
                            ...
                            if($existsCus){
                                # delete
                                $delDetail = array();
                                $this->db->select('id,id_center,id_department,id_group');
                                $this->db->where('mobile', $mobile);
                                $this->db->limit(1);
                    ...
                    if(isset($itemLog) AND !empty($itemLog)){
                        $this->db->insert('referen_log', $itemLog);
