...
<body>
    ...
    <script type="text/javascript">
        function alertcallback(){
            $.ajax({
              url     : '<?php echo base_url();?>staff/getcallback',
              type    : "GET",
              data    : {},
              success : function( data ) {
                var obj_decode = $.parseJSON(data);
                var detail = obj_decode;
                if(detail != null){
                    var id = detail['id'];
                    var callback = detail['callback'];
                    var fullname = detail['fullname'];
                    var mobile = detail['mobile'];

                    var html_callback = '<a href="<?php echo base_url();?>staff/callback/' + id + '" class="">';
                    html_callback += '<span class="glyphicon glyphicon-headphones" aria-hidden="true"></span>' + fullname;
                    html_callback += '</a>';
                    $('#alert_callback').html(html_callback);
                }else{
                    $('#alert_callback').html('');
        ...
        function alertmiscall(){
            $.ajax({
              url     : '<?php echo base_url();?>staff/getmisscall',
              type    : "GET",
              data    : {},
              success : function( data ) {
                var obj_decode = $.parseJSON(data);
                ...
                if(detail != null){
                    var mobile = detail['mobile'];
                    var calldate = detail['calldate'];

                    var html_callback = '<a href="<?php echo base_url();?>staff/call/misscall" class="">';
                    html_callback += '<span class="glyphicon glyphicon-headphones" aria-hidden="true"></span>cuộc gọi nhỡ từ số: ' + mobile + ' Thời gian: ' + calldate;
                    html_callback += '</a>';
                    $('#alert_callback').html(html_callback);
                }else{
                    $('#alert_callback').html('');
        ...
        $(function(){
            //setInterval(alertmiscall, 10*1000);
            //setInterval(alertcallback, 1*60*1000);
        });
    </script>
    <?php } ?>
    ...
    <div class="col-sm-2">
        <?php
            include_once('includes/mn_staff.php');
        ?>
    </div>
    <div class="col-sm-10">
        <?php
    ...
    <?php } else{ 
    if(isset($ishome) AND !empty($ishome)){ ?>
    <script type="text/javascript">
        $(function(){
            setInterval(function(){
                window.location = '<?php echo base_url() . $this->_role . '/startcase' ;?>';
            }, 8 * 1000);
        });
    </script>
    <?php }}?>
